...
@section('content')
    ...
    <section class="content-header">
        ...
        <h1>{{trans('admin.orders')}}
            <small>{{ $orders->total() }} {{trans('admin.orders')}}</small>
        </h1>
            ...
            <div class="col-md-8">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-header">
                        ...
                        <form action="{{ url(route('orders.index')) }}" method="get">

                            <div class="row">

                                <div class="col-md-8">
                                    <input type="text" name="search" class="form-control"
                                           placeholder="{{trans('admin.search')}}" value="{{ request()->search }}">
                                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-primary btn-sm order-products"
                                                    data-url="{{ url(route('orders.products', $order->id)) }}"
                                                    data-method="get"
                                            >
                                            ...
                                            @if (auth()->user()->hasPermission('update_orders'))
                                                <a href="{{ url(route('clients.orders.edit', ['client' => $order->client->id, 'order' => $order->id])) }}"
                                                   class="btn btn-warning btn-sm"><i
                                                        class="fa fa-pencil"></i> {{trans('admin.edit')}}</a>
                                            @else
                                            ...
                                            @if (auth()->user()->hasPermission('delete_orders'))
                                                <form action="{{ url(route('orders.destroy', $order->id)) }}"
                                                      method="post" style="display: inline-block;">
                                                    {{ csrf_field() }}
                                                    {{ method_field('delete') }}
                                                    <button type="submit" class="btn btn-danger btn-sm delete"><i
                                                            class="fa fa-trash"></i> {{trans('admin.delete')}}</button>
                                                </form>

                                            @else
                                                <a href="#" class="btn btn-danger btn-sm" disabled><i
                                                        class="fa fa-trash"></i> {{trans('admin.delete')}}</a>
                                            @endif
